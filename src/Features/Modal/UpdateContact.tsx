...
import {updateContactType} from "./types";
import {asyncActions} from "../Contacts";
import {appActions, useAppDispatch} from "../../costomHooks";



const UpdateContact = React.memo(({showAdd, contact}: updateContactType) => {
    ...
    let formik = useFormik({
        ...
        onSubmit: async (values: ContactModelType) => {
            const res = await dispatch(asyncActions.updateContact({id: contact.id, model: values}))
            if (!asyncActions.updateContact.rejected.match(res)) {
                dispatch(appActions.setAppError({error: 'error. Contact may not be changed'}))
            }
            formik.resetForm()
            formik.errors = {}
            showAdd(false)
